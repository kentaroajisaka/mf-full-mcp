...
マネーフォワード クラウド会計の**非公式**MCPサーバーです。公式beta MCPと同名のツール（`mfc_ca_*`）を提供しつつ、次の2点を足しています。

- **複数事業者への同時接続** — 複数法人分のトークンを保持し、ウィンドウ／セッションごとに別々の事業者へ接続できる
- **公式MCPが提供していない操作** — 証憑のアップロード・添付・添付解除、仕訳の完全削除
...
MFのアクセストークンは、**認可画面で選んだ1事業者に紐づきます**（`/api/v3/offices` が配列ではなく単一オブジェクトを返すのがその証拠です）。

公式MCPはリモートHTTPサーバーで、OAuthはMCPクライアント側が握ります。したがって **サーバー登録1つ＝トークン1つ＝1事業者** が上限になります。2社目を触るには認証をやり直すしかありません。

このサーバーはOAuthを自前で持つことでこの制約を外しています。
...
- **トークンは平文で保存されます。** `~/.mf-full-mcp/tokens.json` にパーミッション`0600`で置かれます。ディスク暗号化された個人端末での利用を前提としています。共用端末では使わないでください。OSのキーチェーンへの移行は今後の課題です。
- **client_secretは存在しません。** 動的クライアント登録により、秘密鍵を保管する必要がない設計です。
- 不要になった事業者のトークンは `remove_office` で削除してください。
